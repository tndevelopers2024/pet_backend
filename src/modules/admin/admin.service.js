...
async function getUsers() {
  ...
  const [boardingCounts, groomingCounts, petCounts] = await Promise.all([
  ...
  const bMap = toMap(boardingCounts), gMap = toMap(groomingCounts), pMap = toMap(petCounts);
...
async function getUserDetail(userId) {
  ...
  const [pets, boarding, grooming] = await Promise.all([
  ...
  return { user, pets, boarding, grooming };
}
...
module.exports = { getDashboardStats, getUsers, getUserDetail, getBoardingDetail, getGroomingDetail, updateBoardingStatus, updateGroomingStatus };
